...
export function FanCoilControls({ equipment, metrics, values, onChange }: FanCoilControlsProps) {
  ...
  const sendCompleteState = async (updatedValues: ControlValues) => {
    try {
      ...
      for (const [key, value] of Object.entries(updatedValues)) {
        // Skip complex objects like pidControllers and outdoorAirReset
  ...
  const handleTemperatureSetpointChange = async (value: number) => {
    ...
    const success = await sendControlCommand("update_temperature_setpoint", numericValue, {
  ...
  const handleApplyChanges = async () => {
    try {
      ...
      // PID Controllers
      if (!isEqual(controlValues.pidControllers, previousControlValues.pidControllers)) {
      ...
      // Outdoor Air Reset
      if (!isEqual(controlValues.outdoorAirReset, previousControlValues.outdoorAirReset)) {
        const success = await sendControlCommand("update_outdoorAirReset", controlValues.outdoorAirReset, {
          previousValue: previousControlValues.outdoorAirReset,
        })
        if (success) successCount++
      }
  ...
  const evaluateCustomLogic = useCallback(() => {
    ...
    try {
      console.log("Creating display-only logic preview...")

      // Instead of evaluating the PID controller logic, just create a visualization
      // based on the current temperatures and setpoints
      const currentTemp =
        sandbox.metrics.roomTemperature || sandbox.metrics.supplyTemperature || sandbox.metrics.Supply || 72

      const setpoint = controlValues.temperatureSetpoint || 72
      const difference = currentTemp - setpoint

      // Determine if heating or cooling would be needed
      let mode = "neutral"
      if (difference > 1) {
        mode = "cooling"
      ...
      // Create a simple expected output for display only
      const result = {
        mode,
        currentTemp,
        setpoint,
        difference,
        coolingValvePosition: mode === "cooling" ? Math.min(100, Math.max(0, difference * 10)) : 0,
        heatingValvePosition: mode === "heating" ? Math.min(100, Math.max(0, -difference * 10)) : 0,
        fanEnabled: true,
        fanSpeed: mode === "neutral" ? "low" : "medium",
        temperatureSetpoint: controlValues.temperatureSetpoint, // ALWAYS preserve the user's setpoint
      }

      return {
        result,
        hasChanges: false, // Set to false to prevent auto-applying the results
        timestamp: Date.now(),
        displayOnly: true,
      }
    } catch (error) {
      console.error("Error creating display logic preview:", error)
      return {
  ...
  }, [controlValues.customLogicEnabled, controlValues.temperatureSetpoint, sandbox.metrics])
      ...
      {isLoading ? (
        ...
        <>
          ...
          <FanCoilGeneralControls
            ...
            showSaveButton={false} // Set to false to hide the button
          />
