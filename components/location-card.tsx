...
export function LocationDetails({ id }: LocationDetailsProps) {
  ...
  // Helper function to get key metrics for the equipment - COMPREHENSIVE VERSION
  const getKeyMetrics = (equipmentItem: any) => {
    ...
    // Look for space/zone/room temperature with various naming conventions
    const spaceTemperatureNames = [
      "SpaceTemp",
      "Space Temp",
      "Space Temperature",
      "SpaceTemperature",
      ...
      <Card>
        ...
        <CardContent>
          {equipment.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {equipment
                .filter((item) => item && (item.name || item.system)) // Filter out items without name OR system
                .sort((a, b) => {
                  const nameA = a.name || a.system || '';
                  const nameB = b.name || b.system || '';
