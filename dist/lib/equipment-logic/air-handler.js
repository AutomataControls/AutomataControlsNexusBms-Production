...
function airHandlerControl(metrics, settings, currentTemp, pidState) {
    ...
    // If in auto mode, determine whether to heat or cool based on room temperature
    if (operationMode === "auto") {
        if (roomTemp < roomSetpoint - deadband) {
            operationMode = "heating";
            console.log("Auto mode selected heating based on room temperature");
        }
        else if (roomTemp > roomSetpoint + deadband) {
            operationMode = "cooling";
            console.log("Auto mode selected cooling based on room temperature");
        }
        else {
            // If room temperature is within deadband, maintain current mode
            console.log("Auto mode - room temperature within deadband, maintaining current state");
        }
    }
    ...
    if (operationMode === "cooling") {
        ...
        if (pidSettings.cooling.enabled) {
            ...
            // Log PID details
            console.log(`PID Controller (cooling): Input=${currentTemp.toFixed(2)}, Setpoint=${supplySetpoint.toFixed(2)}, Error=${(currentTemp - supplySetpoint).toFixed(2)}, P=${coolingPID.p.toFixed(2)}, I=${coolingPID.i.toFixed(2)} (max=${pidSettings.cooling.maxIntegral}), D=${coolingPID.d.toFixed(2)}, Output=${coolingPID.output.toFixed(2)}`);
            ...
            console.log(`Cooling: Temp diff (current - setpoint) = ${tempDiff.toFixed(2)}°F, Valve position = ${coolingValvePosition.toFixed(2)}%`);
            if (tempDiff > 0) {
                console.log(`Supply temp too warm (${currentTemp}°F > ${supplySetpoint}°F): Opening cooling valve to ${coolingValvePosition.toFixed(2)}%`);
            }
            else if (tempDiff < 0) {
                console.log(`Supply temp too cool (${currentTemp}°F < ${supplySetpoint}°F): Closing cooling valve to ${coolingValvePosition.toFixed(2)}%`);
            }
        ...
        if (pidSettings.heating.enabled) {
            ...
            // Log PID details
            console.log(`PID Controller (heating): Input=${currentTemp.toFixed(2)}, Setpoint=${supplySetpoint.toFixed(2)}, Error=${(supplySetpoint - currentTemp).toFixed(2)}, P=${heatingPID.p.toFixed(2)}, I=${heatingPID.i.toFixed(2)} (max=${pidSettings.heating.maxIntegral}), D=${heatingPID.d.toFixed(2)}, Output=${heatingPID.output.toFixed(2)}`);
            ...
            console.log(`Heating: Temp diff (setpoint - current) = ${tempDiff.toFixed(2)}°F, Valve position = ${heatingValvePosition.toFixed(2)}%`);
            if (tempDiff > 0) {
                console.log(`Supply temp too cool (${currentTemp}°F < ${supplySetpoint}°F): Opening heating valve to ${heatingValvePosition.toFixed(2)}%`);
            }
            else if (tempDiff < 0) {
                console.log(`Supply temp too warm (${currentTemp}°F > ${supplySetpoint}°F): Closing heating valve to ${heatingValvePosition.toFixed(2)}%`);
            }
            // Store the current setpoint for change detection
            if (pidState?.heating) {
