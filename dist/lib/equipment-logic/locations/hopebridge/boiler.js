...
function boilerControl(metrics, settings, currentTemp, stateStorage) {
    ...
    (0, location_logger_1.logLocationEquipment)(locationId, equipmentId, "boiler", "Starting Hopebridge-specific boiler control logic");
    ...
    if (outdoorTemp <= 32) {
        setpoint = 155;
        (0, location_logger_1.logLocationEquipment)(locationId, equipmentId, "boiler", `OAR: OAT ${outdoorTemp}°F <= 32°F, using max setpoint: ${setpoint}°F`);
    }
    else if (outdoorTemp >= 72) {
        setpoint = 80;
        (0, location_logger_1.logLocationEquipment)(locationId, equipmentId, "boiler", `OAR: OAT ${outdoorTemp}°F >= 72°F, using min setpoint: ${setpoint}°F`);
    }
    else {
        // Linear interpolation for values between min and max
        const ratio = (outdoorTemp - 32) / (72 - 32);
        setpoint = 155 - ratio * (155 - 80);
        (0, location_logger_1.logLocationEquipment)(locationId, equipmentId, "boiler", `OAR: Calculated setpoint: ${setpoint.toFixed(1)}°F (ratio: ${ratio.toFixed(2)})`);
    }
    ...
    if (groupId) {
        ...
        (0, location_logger_1.logLocationEquipment)(locationId, equipmentId, "boiler", "Boiler not part of a group, defaulting to LEAD");
    }
    // STEP 8: Call base implementation with our modified settings
    (0, location_logger_1.logLocationEquipment)(locationId, equipmentId, "boiler", "Calling base implementation with Hopebridge-specific settings");
    const result = (0, boiler_1.boilerControl)(metrics, hopebridgeSettings, currentTemp, stateStorage);
