...
exports.getHuntingtonEquipmentGroup = getHuntingtonEquipmentGroup;
exports.checkHuntingtonBoilerHealth = checkHuntingtonBoilerHealth;
exports.getHuntingtonLeadLagStatus = getHuntingtonLeadLagStatus;
exports.getHuntingtonPumpLeadLagStatus = getHuntingtonPumpLeadLagStatus;
exports.checkHuntingtonPumpHealth = checkHuntingtonPumpHealth;
// @ts-nocheck
 ...
 */
function getHuntingtonEquipmentGroup(equipmentId) {
    (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", "Checking Huntington equipment groups");
    ...
    for (const [groupKey, group] of Object.entries(HUNTINGTON_EQUIPMENT_GROUPS)) {
        if (group.equipmentIds.includes(equipmentId)) {
            (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `Found in Huntington group: ${group.name} (${group.equipmentIds.length} units)`);
            return group;
        }
    }
    (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", "Not found in any Huntington equipment group");
    return null;
}
/**
 * Check Huntington boiler health based on InfluxDB metrics
 */
async function checkHuntingtonBoilerHealth(equipmentId, metrics) {
    (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", "Checking Huntington boiler health");
    try {
        ...
        if (supplyTemp > 170) {
            (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `UNHEALTHY: Supply temperature ${supplyTemp}°F exceeds safe limit (170°F)`);
            return false;
        }
        ...
        if (freezestat === true || freezestat === "true" || freezestat === 1) {
            (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `UNHEALTHY: Freezestat condition detected`);
            return false;
        }
        ...
        if (boilerStatus.toLowerCase().includes("fault") || boilerStatus.toLowerCase().includes("error")) {
            (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `UNHEALTHY: Boiler status indicates fault: ${boilerStatus}`);
            return false;
        }
        ...
        if (outdoorTemp < 60 && supplyTemp < 100 && firing === 0) {
            ...
            (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `WARNING: Cold weather but boiler not firing (OAT: ${outdoorTemp}°F, Supply: ${supplyTemp}°F)`);
            // Don't mark as unhealthy yet, but this could be monitored
        }
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `HEALTHY: Supply: ${supplyTemp}°F, Status: ${boilerStatus}, Firing: ${firing}`);
        return true;
    }
    catch (error) {
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `Error checking boiler health: ${error.message} - defaulting to healthy`);
 ...
 */
async function getHuntingtonLeadLagStatus(equipmentId, metrics, stateStorage) {
    (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", "Determining Huntington lead-lag status");
    ...
    if (!group) {
        ...
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", "Not in Huntington equipment group - running independently");
        return {
            isLead: true,
            shouldRun: true,
    ...
    if (!group.useLeadLag) {
        ...
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", "Lead-lag disabled for Huntington equipment - running independently");
        return {
            isLead: true,
            shouldRun: true,
    ...
    if (now - stateStorage.huntingtonLeadLag.lastHealthCheck > 30000) {
        ...
        if (group.autoFailover && currentLeadId !== equipmentId) {
            ...
            if (!leadEquipmentHealthy) {
                ...
                (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `FAILOVER: Lead equipment ${currentLeadId} failed, promoting ${equipmentId} to lead`);
    ...
    if (group.useLeadLag && now - stateStorage.huntingtonLeadLag.lastRotationCheck > 300000) {
        ...
        if (daysSinceLastRotation >= group.changeoverIntervalDays) {
            ...
            if (nextLeadId && nextLeadId !== currentLeadId) {
                ...
                (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `ROTATION: Scheduled changeover from ${currentLeadId} to ${nextLeadId} (${daysSinceLastRotation.toFixed(1)} days since last rotation)`);
    ...
    if (isLead) {
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `LEAD EQUIPMENT: Operating as lead in Huntington group ${group.name}`);
        return {
            isLead: true,
            shouldRun: true,
        ...
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "boiler", `LAG EQUIPMENT: Standing by as lag in Huntington group ${group.name} (lead: ${finalLeadId})`);
        return {
            isLead: false,
            shouldRun: false,
 ...
 */
async function getHuntingtonPumpLeadLagStatus(equipmentId, pumpType, metrics, stateStorage) {
    (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", "Determining Huntington pump lead-lag status");
    ...
    if (!group) {
        // Not in a group - run independently
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", "Not in Huntington pump group - running independently");
        return {
            isLead: true,
            shouldRun: true,
            reason: "Not in lead-lag group - running independently",
    ...
    if (!group.useLeadLag) {
        ...
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", "Lead-lag disabled for Huntington pumps - running independently");
        return {
            isLead: true,
            shouldRun: true,
            reason: "Lead-lag disabled - running independently",
    ...
    if (now - stateStorage[stateKey].lastHealthCheck > 30000) {
        ...
        if (group.autoFailover && currentLeadId !== equipmentId) {
            ...
            if (!leadPumpHealthy) {
                ...
                (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `FAILOVER: Lead pump ${currentLeadId} failed, promoting ${equipmentId} to lead`);
                // Write failover event to InfluxDB
    ...
    if (group.useLeadLag && now - stateStorage[stateKey].lastRotationCheck > 300000) {
        ...
        if (daysSinceLastRotation >= group.changeoverIntervalDays) {
            ...
            if (nextLeadId && nextLeadId !== currentLeadId) {
                ...
                (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `ROTATION: Scheduled changeover from ${currentLeadId} to ${nextLeadId} (${daysSinceLastRotation.toFixed(1)} days since last rotation)`);
                // Write rotation event to InfluxDB
                await writeHuntingtonLeadLagEvent(group.id, nextLeadId, "rotation", `Scheduled rotation after ${daysSinceLastRotation.toFixed(1)} days`);
            }
        }
    ...
    const isLead = finalLeadId === equipmentId;
    if (isLead) {
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `LEAD PUMP: Operating as lead in Huntington group ${group.name}`);
        return {
            isLead: true,
            shouldRun: true,
        ...
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `LAG PUMP: Standing by as lag in Huntington group ${group.name} (lead: ${finalLeadId})`);
        return {
            isLead: false,
            shouldRun: false,
 ...
 */
async function checkHuntingtonPumpHealth(equipmentId, pumpType, metrics) {
    (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", "Checking Huntington pump health");
    try {
        let pumpAmps = 0;
        ...
        if (pumpStatus.toLowerCase().includes("fault") || pumpStatus.toLowerCase().includes("error")) {
            (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `UNHEALTHY: Pump status indicates fault: ${pumpStatus}`);
            return false;
        }
        ...
        if (pumpAmps < 0.5 && pumpStatus !== "off") {
            (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `UNHEALTHY: Low amp draw (${pumpAmps}A) when pump should be running (status: ${pumpStatus})`);
            return false;
        }
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `HEALTHY: Amps: ${pumpAmps}A, Status: ${pumpStatus}`);
        return true;
    }
    catch (error) {
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `Error checking pump health: ${error.message} - defaulting to healthy`);
        return true; // Default to healthy if we can't determine
    }
}
/**
 ...
 */
async function writeHuntingtonLeadLagEvent(groupId, equipmentId, eventType, reason) {
    try {
        ...
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `Huntington lead-lag event logged: ${eventType} - ${reason}`);
    }
    catch (error) {
        (0, location_logger_1.logLocationEquipment)("4", equipmentId, "pump", `Failed to log Huntington lead-lag event: ${error.message}`);
    }
}
