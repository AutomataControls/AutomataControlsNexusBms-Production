...
function airHandlerControl(metrics, settings, currentTemp, pidState) {
    ...
    if (operationMode === "cooling") {
        ...
        if (pidSettings.cooling.enabled) {
            ...
            console.log(`PID Controller (cooling): Input=${currentTemp.toFixed(2)}, Setpoint=${supplySetpoint.toFixed(2)}, Error=${(currentTemp - supplySetpoint).toFixed(2)}, P=${coolingPID.p?.toFixed(2) || "N/A"}, I=${coolingPID.i?.toFixed(2) || "N/A"} (max=${pidSettings.cooling.maxIntegral}), D=${coolingPID.d?.toFixed(2) || "N/A"}, Output=${coolingPID.output?.toFixed(2) || coolingPID.output || "N/A"}`);
            ...
            console.log(`Cooling: Temp diff (current - setpoint) = ${tempDiff.toFixed(2)}°F, Valve position = ${coolingValvePosition?.toFixed(2) || coolingValvePosition || "N/A"}%`);
            if (tempDiff > 0) {
                console.log(`Supply temp too warm (${currentTemp}°F > ${supplySetpoint}°F): Opening cooling valve to ${coolingValvePosition?.toFixed(2) || coolingValvePosition || "N/A"}%`);
            }
            else if (tempDiff < 0) {
                console.log(`Supply temp too cool (${currentTemp}°F < ${supplySetpoint}°F): Closing cooling valve to ${coolingValvePosition?.toFixed(2) || coolingValvePosition || "N/A"}%`);
            }
        ...
        if (pidSettings.heating.enabled) {
            ...
            // FIXED: Added optional chaining and fallbacks to avoid "cannot read property of undefined" errors
            console.log(`PID Controller (heating): Input=${currentTemp.toFixed(2)}, Setpoint=${supplySetpoint.toFixed(2)}, Error=${(supplySetpoint - currentTemp).toFixed(2)}, P=${heatingPID.p?.toFixed(2) || "N/A"}, I=${heatingPID.i?.toFixed(2) || "N/A"} (max=${pidSettings.heating.maxIntegral}), D=${heatingPID.d?.toFixed(2) || "N/A"}, Output=${heatingPID.output?.toFixed(2) || heatingPID.output || "N/A"}`);
            ...
            console.log(`Heating: Temp diff (setpoint - current) = ${tempDiff.toFixed(2)}°F, Valve position = ${heatingValvePosition?.toFixed(2) || heatingValvePosition || "N/A"}%`);
            if (tempDiff > 0) {
                console.log(`Supply temp too cool (${currentTemp}°F < ${supplySetpoint}°F): Opening heating valve to ${heatingValvePosition?.toFixed(2) || heatingValvePosition || "N/A"}%`);
            }
            else if (tempDiff < 0) {
                console.log(`Supply temp too warm (${currentTemp}°F > ${supplySetpoint}°F): Closing heating valve to ${heatingValvePosition?.toFixed(2) || heatingValvePosition || "N/A"}%`);
            }
            // Store the current setpoint for change detection
            if (pidState?.heating) {
