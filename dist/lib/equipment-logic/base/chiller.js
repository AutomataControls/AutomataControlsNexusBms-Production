    ...
    try {
        ...
        if (isDualChillerLocation) {
            ...
            if (isLead) {
                ...
                console.log(`Lag chiller ${equipmentId} enable: ${result.unitEnable}, lead issues: ${leadChillerHasIssues}, needs additional cooling: ${needsAdditionalCooling}`);
            }
        }
        else {
 ...
 */
function getChillerNumber(equipmentId) {
    try {
 ...
 */
function getCustomLockoutTemperature(metrics) {
 ...
 */
function hasWaterTemperatureControl(metrics, settings) {
 ...
 */
function determineLeadLag(locationId, chillerNumber, stateStorage) {
    const now = Date.now();
 ...
 * Update runtime tracking
 */
function updateRuntimeTracking(equipmentId, stateStorage, isRunning) {
    const now = Date.now();
 ...
 */
function checkForLeadChillerIssues(metrics, locationId, stateStorage) {
 ...
 */
function checkIfNeedsAdditionalCooling(currentTemp, setpoint) {
 ...
 */
function determineEnableBasedOnWaterTemp(currentTemp, setpoint) {
 ...
 */
function checkForAlarms(metrics) {
