...
function boilerControl(metrics, settings, currentTemp, stateStorage) {
    ...
    const supplyTemp = parseTemperature(metrics.H20Supply) || // Primary field name from InfluxDB - EXACT MATCH (note: H20 not H2O)
    ...
    const returnTemp = parseTemperature(metrics.H20Return) || // Primary field name from InfluxDB - EXACT MATCH (note: H20 not H2O)
    ...
    const outdoorTemp = parseTemperature(metrics.outdoorTemp) || // Primary field name from InfluxDB - EXACT MATCH
 ...
 */
function checkForLocationSpecificResults(settings, stateStorage) {
