...
function pumpControl(metrics, settings, currentTemp, stateStorage) {
    ...
    // MODIFICATION: Check for overridePumpNumber setting first
    const pumpNumber = settings.overridePumpNumber !== undefined ?
        settings.overridePumpNumber :
        getPumpNumber(equipmentId);
    ...
    // CRITICAL FIX: Check if lead status is being forced by a location-specific implementation
    // This allows location-specific implementations to override the lead status
    if (settings.forceLeadStatus !== undefined) {
        isLeadPump = !!settings.forceLeadStatus;
        console.log(`Using forced lead status for ${equipmentId}: ${isLeadPump ? "LEAD" : "LAG"}`);
    }
    // If not forced, use the standard lead/lag determination
    else if (pumpGroupId) {
 ...
 */
function getPumpNumber(equipmentId) {
 ...
 */
function getAmpReading(metrics, pumpType, pumpNumber) {
    ...
    for (const key of keys) {
        ...
        if ((key.toLowerCase().includes(String(pumpNumber)) ||
 ...
 * Check if location is exempt from outdoor temperature lockouts
 */
function checkLocationExemptFromLockout(locationId) {
 ...
 */
function checkLeadPumpIssue(metrics, pumpType, groupOrLocationId) {
    // Look for fields that might indicate lead pump status
    const keys = Object.keys(metrics);
    for (const key of keys) {
        if ((key.toLowerCase().includes("lead") || key.toLowerCase().includes("pump1") || key.toLowerCase().includes("pump_1")) &&
 ...
 */
function checkPumpAlarm(metrics) {
