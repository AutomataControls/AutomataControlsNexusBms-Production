...
exports.INFLUXDB_RETRY_DELAY = exports.INFLUXDB_MAX_RETRIES = exports.INFLUXDB_TIMEOUT = exports.INFLUXDB_DATABASE = exports.INFLUXDB_URL = void 0;
exports.queryInfluxDB = queryInfluxDB;
exports.writeToInfluxDB = writeToInfluxDB;
exports.formatLineProtocol = formatLineProtocol;
exports.createTimeRangeQuery = createTimeRangeQuery;
exports.getLatestValues = getLatestValues;
exports.getEquipmentMetrics = getEquipmentMetrics;
exports.getEquipmentControlValues = getEquipmentControlValues;
exports.writeControlCommand = writeControlCommand;
// Configuration from environment with fallbacks
exports.INFLUXDB_URL = process.env.INFLUXDB_URL || 'http://143.198.162.31:8181';
exports.INFLUXDB_DATABASE = process.env.INFLUXDB_DATABASE || 'Locations';
exports.INFLUXDB_TIMEOUT = parseInt(process.env.INFLUXDB_TIMEOUT || '30000'); // 30 seconds
exports.INFLUXDB_MAX_RETRIES = parseInt(process.env.INFLUXDB_MAX_RETRIES || '3');
exports.INFLUXDB_RETRY_DELAY = parseInt(process.env.INFLUXDB_RETRY_DELAY || '1000'); // 1 second
/**
...
async function queryInfluxDB(query, options = {}) {
    const maxRetries = options.maxRetries || exports.INFLUXDB_MAX_RETRIES;
    const retryDelay = options.retryDelay || exports.INFLUXDB_RETRY_DELAY;
    const timeout = options.timeout || exports.INFLUXDB_TIMEOUT;
 ...
 */
async function writeToInfluxDB(lineProtocol, options = {}) {
    const maxRetries = options.maxRetries || exports.INFLUXDB_MAX_RETRIES;
    const retryDelay = options.retryDelay || exports.INFLUXDB_RETRY_DELAY;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const response = await fetch(`${exports.INFLUXDB_URL}/api/v3/write_lp?db=${exports.INFLUXDB_DATABASE}&precision=${precision}`, {
                method: 'POST',
                headers: {
 ...
 */
function formatLineProtocol(measurement, tags, fields, timestamp) {
    // Validate inputs
    if (!measurement) {
 ...
 */
function createTimeRangeQuery(measurement, fields = ['*'], filters = '', timeRange = 24, limit = 1000) {
    // Validate inputs
    if (!measurement) {
        throw new Error('Measurement name is required');
    }
 ...
 */
async function getLatestValues(measurement, tags, options = {}) {
    // Build tag filters
    let tagFilters = '';
 ...
 */
async function getEquipmentMetrics(equipmentId, locationId) {
    const filters = [];
    ...
    const query = createTimeRangeQuery('metrics', ['*'], filters.join(' AND '), 1, // Last hour
 ...
 */
async function getEquipmentControlValues(equipmentId, locationId) {
    ...
    `;
        ...
        promises.push(queryInfluxDB(valueQuery)
            .then(result => {
            if (result.success && result.data && Array.isArray(result.data) && result.data.length > 0) {
 ...
 */
async function writeControlCommand(command, data) {
    try {
