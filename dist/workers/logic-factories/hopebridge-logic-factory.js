"use strict";
// ===============================================================================
// Hopebridge Logic Factory Worker
// ===============================================================================
//
...
//
// PURPOSE:
// Consumes equipment jobs from the BullMQ queue and processes Hopebridge location
...
Object.keys(require.cache).forEach(key => {
    if (key.includes('/hopebridge/') || key.includes('equipment-logic')) {
...
const clearCache = async () => {
    try {
        const keys = await connection.keys('*hopebridge*');
        const locationKeys = await connection.keys('*location-5*');
        const equipmentKeys = await connection.keys('*equipment-logic-5*');
        ...
        if (allKeys.length > 0) {
            ...
            console.log(`[Hopebridge Logic Factory] Cleared ${allKeys.length} cache keys`);
        }
    } catch (error) {
        console.error('[Hopebridge Logic Factory] Cache clear error:', error);
    }
};
...
// Hopebridge equipment logic path
const EQUIPMENT_PATH = '/opt/productionapp/dist/lib/equipment-logic/locations/hopebridge';

// Hopebridge Logic Factory Worker
const hopebridgeLogicWorker = new Worker(
    'equipment-logic-5',
    async (job) => {
        const { equipmentId, locationId, type: equipmentType } = job.data;

        // Only process Hopebridge equipment (location ID = 5)
        if (locationId !== '5') {
            console.log(`[Hopebridge Logic Factory] Skipping non-Hopebridge equipment: ${equipmentId}`);
            return;
        }

        console.log(`[Hopebridge Logic Factory] Processing ${equipmentType} (${equipmentId})...`);
        ...
        try {
            ...
            console.log(`[Hopebridge Logic Factory] Completed ${equipmentType} (${equipmentId}) successfully`);
            return { success: true, equipmentId, results: results.length };

        } catch (error) {
            console.error(`[Hopebridge Logic Factory] Error processing ${equipmentType} (${equipmentId}):`, error);
            throw error;
        }
    },
    {
...
// Map equipment types to files - HOPEBRIDGE EQUIPMENT
function getEquipmentFile(equipmentType) {
    const typeMapping = {
        // HOPEBRIDGE AIR HANDLERS
        'air-handler-1': 'air-handler.js',
        'air-handler-2': 'air-handler.js',  // INCLUDES AHU-2!
        'air-handler-3': 'air-handler.js',
    ...
    if (!mappedFile) {
        console.log(`[Hopebridge Logic Factory] Unknown equipment type: ${equipmentType}`);
        console.log(`[Hopebridge Logic Factory] Available types:`, Object.keys(typeMapping));
        return null;
    }

    console.log(`[Hopebridge Logic Factory] Mapping ${equipmentType} → ${mappedFile}`);
    return mappedFile;
}
...
async function executeEquipmentLogic(fileName, equipmentId, equipmentType) {
    try {
        ...
        console.log(`[Hopebridge Logic Factory] Calling ${fileName} with 4 parameters:`);
        ...
        console.log(`[Hopebridge Logic Factory] Equipment logic returned:`, resultsArray);

        return resultsArray;

    } catch (error) {
        console.error(`[Hopebridge Logic Factory] Logic execution error:`, error);
        return [];
    }
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (Array.isArray(data) && data.length > 0) {
                ...
                console.log(`[Hopebridge Logic Factory] Gathered ${Object.keys(metricsObject).length} metrics for equipment ${equipmentId}`);
                return metricsObject;
            }
        }

        console.log(`[Hopebridge Logic Factory] No recent metrics found for equipment ${equipmentId}, using defaults`);
        return {
            Outdoor_Air: 50,
            RoomTemp: 72,
            SupplyTemp: 55,
        ...
        console.error(`[Hopebridge Logic Factory] Error gathering metrics for ${equipmentId}:`, error);
        return {
            Outdoor_Air: 50,
            RoomTemp: 72,
            SupplyTemp: 55,
...
async function gatherSettingsData(fileName, equipmentId, equipmentType) {
    try {
        ...
        console.log(`[Hopebridge Logic Factory] Settings for ${fileName}: equipmentId=${equipmentId}`);
        return settingsInput;

    } catch (error) {
        console.error(`[Hopebridge Logic Factory] Error gathering settings:`, error);
        return {
            equipmentId: equipmentId,
            locationId: '5',
            locationName: 'hopebridge'
...
function getCurrentTemperature(metricsInput, equipmentType) {
    // For boilers, prioritize water supply temperature
    if (equipmentType && equipmentType.includes('boiler')) {
        const boilerTempSources = ['H20Supply', 'H20_Supply', 'H20 Supply', 'SupplyTemp', 'Supply_Temp'];
        for (const source of boilerTempSources) {
            if (metricsInput[source] !== undefined && !isNaN(parseFloat(metricsInput[source]))) {
                const temp = parseFloat(metricsInput[source]);
                console.log(`[Hopebridge Logic Factory] Using boiler supply temp from metrics.${source}: ${temp}°F for ${equipmentType}`);
                return temp;
            }
        }
    } else {
        // For air handlers and pumps, use room/space temperature
        const tempSources = [
        ...
        for (const source of tempSources) {
            if (metricsInput[source] !== undefined && !isNaN(parseFloat(metricsInput[source]))) {
                const temp = parseFloat(metricsInput[source]);
                console.log(`[Hopebridge Logic Factory] Using temperature from metrics.${source}: ${temp}°F for ${equipmentType}`);
                return temp;
            }
        }
    }

    console.log(`[Hopebridge Logic Factory] No temperature found for ${equipmentType}, using default: 72°F`);
    return 72;
}
...
async function gatherStateStorage() {
    try {
        ...
        console.error(`[Hopebridge Logic Factory] Error gathering state storage:`, error);
        return {};
    }
}

// Extract actionable commands from results - Hopebridge specific
function extractActionableCommands(result) {
    const actionable = {};

    // AIR HANDLER COMMANDS (including AHU-2!)
    ...
    // AHU-2 DX COOLING COMMANDS
    if (result.dxEnabled !== undefined) actionable.dxEnabled = result.dxEnabled;

    // AHU-1 CHILLER COMMANDS
    if (result.chillerEnabled !== undefined) actionable.chillerEnabled = result.chillerEnabled;
    if (result.cwCircPumpEnabled !== undefined) actionable.cwCircPumpEnabled = result.cwCircPumpEnabled;
...
async function writeEquipmentResults(equipmentType, results, equipmentId) {
    try {
        ...
        console.log(`[Hopebridge Logic Factory] Writing results to ${database} for ${equipmentType} (${equipmentId})`);
        ...
        for (const result of results) {
            ...
            if (!result || typeof result !== 'object') {
                console.log(`[Hopebridge Logic Factory] Skipping invalid result:`, result);
                continue;
            }

            const actionableCommands = extractActionableCommands(result);
            if (Object.keys(actionableCommands).length === 0) {
                console.log(`[Hopebridge Logic Factory] No actionable commands found in result:`, result);
                continue;
            }

            console.log(`[Hopebridge Logic Factory] Extracted ${Object.keys(actionableCommands).length} commands:`,
            ...
            for (const [commandType, value] of Object.entries(actionableCommands)) {
                ...
                const lineProtocol = `NeuralCommands,equipment_id=${equipmentId},location_id=5,command_type=${commandType},equipment_type=${equipmentType},source=hopebridge-logic-factory,status=active value=${formattedValue}`;
            ...
            if (lineProtocolEntries.length > 0) {
                ...
                try {
                    ...
                    if (response.ok) {
                        console.log(`[Hopebridge Logic Factory] Successfully wrote ${lineProtocolEntries.length} commands to ${database}`);
                    } else {
                        const errorText = await response.text();
                        console.error(`[Hopebridge Logic Factory] Write failed: ${response.status} - ${errorText}`);
                    }
                } catch (fetchError) {
                    console.error(`[Hopebridge Logic Factory] Fetch error writing to ${database}:`, fetchError);
                }
            }
        }
    } catch (error) {
        console.error(`[Hopebridge Logic Factory] Write error:`, error);
    }
}

// Worker event handlers
hopebridgeLogicWorker.on('completed', (job) => {
    console.log(`[Hopebridge Logic Factory] Job ${job.id} completed successfully`);
});

hopebridgeLogicWorker.on('failed', (job, err) => {
    console.error(`[Hopebridge Logic Factory] Job ${job.id} failed:`, err);
});

hopebridgeLogicWorker.on('error', (err) => {
    console.error('[Hopebridge Logic Factory] Worker error:', err);
});

// Graceful shutdown
async function shutdown() {
    console.log('[Hopebridge Logic Factory] Shutting down...');
    await hopebridgeLogicWorker.close();
...
console.log('[Hopebridge Logic Factory] Hopebridge Logic Factory Worker started - Processing autism therapy center equipment');
console.log('[Hopebridge Logic Factory] Equipment supported: air-handler-1, air-handler-2, air-handler-3, boiler-1, boiler-2, hw-pump-1, hw-pump-2');
console.log('[Hopebridge Logic Factory] Queue: equipment-logic-5, Concurrency: 5, Location ID: 5');
console.log('[Hopebridge Logic Factory] Redis connected');
