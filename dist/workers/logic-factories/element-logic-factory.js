"use strict";
// ===============================================================================
// Element Logic Factory Worker
// ===============================================================================
//
// Author: Juelz NeuralBms DevOps
// Last Updated: June 5, 2025
//
// PURPOSE:
// Consumes equipment jobs from the BullMQ queue and processes Element location
// equipment logic using the 4-parameter interface.
//
// ARCHITECTURE:
// Queue Consumer → Equipment Logic → InfluxDB Results
//
// ===============================================================================

const { Worker } = require('bullmq');
const fs = require('fs');
const path = require('path');
const Redis = require('ioredis');

// Redis connection for BullMQ
const connection = new Redis({
    host: 'localhost',
    port: 6379,
    maxRetriesPerRequest: null,
});

// Clear require cache on startup
Object.keys(require.cache).forEach(key => {
    if (key.includes('/element/') || key.includes('equipment-logic')) {
        delete require.cache[key];
    }
});

// Clear Redis cache synchronously on startup
const clearCache = async () => {
    try {
        const keys = await connection.keys('*element*');
        const locationKeys = await connection.keys('*location-8*');
        const equipmentKeys = await connection.keys('*equipment-logic-8*');
        const queueKeys = await connection.keys('*already-queued*');

        const allKeys = [...keys, ...locationKeys, ...equipmentKeys, ...queueKeys];
        if (allKeys.length > 0) {
            await connection.del(...allKeys);
            console.log(`[Element Logic Factory] Cleared ${allKeys.length} cache keys`);
        }
    } catch (error) {
        console.error('[Element Logic Factory] Cache clear error:', error);
    }
};

// Clear cache before starting worker
clearCache();

// Element equipment logic path
const EQUIPMENT_PATH = '/opt/productionapp/dist/lib/equipment-logic/locations/element';

// Element Logic Factory Worker
const elementLogicWorker = new Worker(
    'equipment-logic-8',
    async (job) => {
        const { equipmentId, locationId, type: equipmentType } = job.data;

        // Only process Element equipment (location ID = 8)
        if (locationId !== '8') {
            console.log(`[Element Logic Factory] Skipping non-Element equipment: ${equipmentId}`);
            return;
        }

        console.log(`[Element Logic Factory] Processing ${equipmentType} (${equipmentId})...`);

        try {
            // Determine equipment file based on type
            const equipmentFile = getEquipmentFile(equipmentType);
            if (!equipmentFile) {
                throw new Error(`Unknown equipment type: ${equipmentType}`);
            }

            // Execute equipment logic with 4-parameter interface
            const results = await executeEquipmentLogic(equipmentFile, equipmentId, equipmentType);

            // Write results to InfluxDB
            await writeEquipmentResults(equipmentType, results, equipmentId);

            console.log(`[Element Logic Factory] Completed ${equipmentType} (${equipmentId}) successfully`);
            return { success: true, equipmentId, results: results.length };

        } catch (error) {
            console.error(`[Element Logic Factory] Error processing ${equipmentType} (${equipmentId}):`, error);
            throw error;
        }
    },
    {
        connection,
        concurrency: 3,
        removeOnComplete: 50,
        removeOnFail: 25,
        settings: {
            stalledInterval: 300000,
            maxStalledCount: 3,
            retryProcessDelay: 5000,
        }
    }
);

// Map equipment types to files - ELEMENT EQUIPMENT
function getEquipmentFile(equipmentType) {
    const typeMapping = {
        // ELEMENT DOAS UNITS
        'doas-1': 'doas.js',
        'doas-2': 'doas.js'
    };

    const mappedFile = typeMapping[equipmentType];

    if (!mappedFile) {
        console.log(`[Element Logic Factory] Unknown equipment type: ${equipmentType}`);
        console.log(`[Element Logic Factory] Available types:`, Object.keys(typeMapping));
        return null;
    }

    console.log(`[Element Logic Factory] Mapping ${equipmentType} → ${mappedFile}`);
    return mappedFile;
}

// Execute equipment logic with proper 4-parameter interface
async function executeEquipmentLogic(fileName, equipmentId, equipmentType) {
    try {
        const filePath = path.join(EQUIPMENT_PATH, fileName);

        // Dynamic import the equipment module
        delete require.cache[require.resolve(filePath)];
        const equipmentModule = require(filePath);

        // Find the appropriate equipment function
        let equipmentFunction = null;

        if (typeof equipmentModule.default === 'function') {
            equipmentFunction = equipmentModule.default;
        } else if (typeof equipmentModule.processEquipment === 'function') {
            equipmentFunction = equipmentModule.processEquipment;
        } else if (typeof equipmentModule.runLogic === 'function') {
            equipmentFunction = equipmentModule.runLogic;
        } else if (typeof equipmentModule.doasControl === 'function') {
            equipmentFunction = equipmentModule.doasControl;
        }

        if (!equipmentFunction) {
            throw new Error(`No equipment function found in ${fileName}`);
        }

        // STEP 1: Gather metricsInput from InfluxDB for THIS specific equipment
        const metricsInput = await gatherMetricsData(equipmentId);

        // STEP 2: Gather settingsInput using the real equipment ID
        const settingsInput = await gatherSettingsData(fileName, equipmentId, equipmentType);

        // STEP 3: Get currentTempArgument from metrics based on equipment type
        const currentTempArgument = getCurrentTemperature(metricsInput, equipmentType);

        // STEP 4: Get state storage
        const stateStorageInput = await gatherStateStorage();

        console.log(`[Element Logic Factory] Calling ${fileName} with 4 parameters:`);
        console.log(`  - metricsInput: ${Object.keys(metricsInput).length} metrics for ${equipmentId}`);
        console.log(`  - settingsInput: equipmentId=${settingsInput.equipmentId}`);
        console.log(`  - currentTempArgument: ${currentTempArgument}`);
        console.log(`  - stateStorageInput: ${Object.keys(stateStorageInput).length} state items`);

        // Execute the equipment logic with proper 4 parameters
        const results = await equipmentFunction(metricsInput, settingsInput, currentTempArgument, stateStorageInput);

        // Ensure results is an array and log what we got back
        const resultsArray = Array.isArray(results) ? results : [results];
        console.log(`[Element Logic Factory] Equipment logic returned:`, resultsArray);

        return resultsArray;

    } catch (error) {
        console.error(`[Element Logic Factory] Logic execution error:`, error);
        return [];
    }
}

// Helper function to gather current metrics from InfluxDB
async function gatherMetricsData(equipmentId) {
    try {
        const database = process.env.INFLUXDB_DATABASE || 'Locations';
        const influxUrl = process.env.INFLUXDB_URL || 'http://143.198.162.31:8181';

        const query = `
            SELECT *
            FROM metrics
            WHERE "equipmentId" = '${equipmentId}'
            AND location_id = '8'
            AND time >= now() - INTERVAL '15 minutes'
            ORDER BY time DESC
            LIMIT 100
        `;

        const response = await fetch(`${influxUrl}/api/v3/query_sql`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ q: query, db: database })
        });

        if (response.ok) {
            const data = await response.json();
            if (Array.isArray(data) && data.length > 0) {
                const metricsObject = {};
                const skipFields = ['equipmentId', 'location_id', 'time', 'equipment_type', 'system', 'zone'];

                for (const metric of data) {
                    Object.entries(metric).forEach(([key, value]) => {
                        if (!skipFields.includes(key) && value !== null && value !== undefined && value !== '') {
                            if (!metricsObject[key]) {
                                metricsObject[key] = value;
                            }
                        }
                    });

                    if (metric.equipmentId) metricsObject.equipmentId = metric.equipmentId;
                    if (metric.location_id) metricsObject.locationId = metric.location_id;
                }

                console.log(`[Element Logic Factory] Gathered ${Object.keys(metricsObject).length} metrics for equipment ${equipmentId}`);
                return metricsObject;
            }
        }

        console.log(`[Element Logic Factory] No recent metrics found for equipment ${equipmentId}, using defaults`);
        return {
            Outdoor_Air: 65,
            SupplyTemp: 65,
            Supply_Air_Temp: 65,
            equipmentId: equipmentId,
            locationId: '8'
        };

    } catch (error) {
        console.error(`[Element Logic Factory] Error gathering metrics for ${equipmentId}:`, error);
        return {
            Outdoor_Air: 65,
            SupplyTemp: 65,
            Supply_Air_Temp: 65,
            equipmentId: equipmentId,
            locationId: '8'
        };
    }
}

// Helper function to gather settings data
async function gatherSettingsData(fileName, equipmentId, equipmentType) {
    try {
        // Check for UI setpoint overrides
        let uiSetpoint = null;
        try {
            const database = 'UIControlCommands';
            const influxUrl = process.env.INFLUXDB_URL || 'http://143.198.162.31:8181';

            const query = `
                SELECT waterTemperatureSetpoint
                FROM UIControlCommands
                WHERE equipmentId = '${equipmentId}'
                AND time >= now() - INTERVAL '1 hour'
                ORDER BY time DESC
                LIMIT 1
            `;

            const uiResponse = await fetch(`${influxUrl}/api/v3/query_sql`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ q: query, db: database })
            });

            if (uiResponse.ok) {
                const uiData = await uiResponse.json();
                if (Array.isArray(uiData) && uiData.length > 0 && uiData[0].waterTemperatureSetpoint) {
                    uiSetpoint = parseFloat(uiData[0].waterTemperatureSetpoint);
                    console.log(`[Element Logic Factory] Found UI setpoint override: ${uiSetpoint}°F for ${equipmentType}`);
                }
            }
        } catch (uiError) {
            console.log(`[Element Logic Factory] No UI setpoint found for ${equipmentType}`);
        }

        const settingsInput = {
            equipmentId: equipmentId,
            locationId: '8',
            locationName: 'element',
            equipmentType: fileName.replace(/\.js$/, ''),
            enabled: true,
            supplyAirSetpoint: equipmentType === 'doas-1' ? 68 : 65,
            waterTemperatureSetpoint: uiSetpoint,
            heatingLockoutTemp: 65,
            coolingLockoutTemp: 50,
            highTempLimit: 85,
            lowTempLimit: 45,
            fanMinSpeed: 20,
            fanMaxSpeed: 100
        };

        console.log(`[Element Logic Factory] Settings for ${fileName}: equipmentId=${equipmentId}`);
        return settingsInput;

    } catch (error) {
        console.error(`[Element Logic Factory] Error gathering settings:`, error);
        return {
            equipmentId: equipmentId,
            locationId: '8',
            locationName: 'element'
        };
    }
}

// Helper function to extract current temperature
function getCurrentTemperature(metricsInput, equipmentType) {
    // For DOAS units, use supply air temperature
    const doasTempSources = [
        'SupplyTemp', 'Supply_Air_Temp', 'SupplyAirTemp',
        'Supply_Temp', 'Supply Temperature',
        'Outdoor_Air', 'OutdoorTemp', 'currentTemp'
    ];

    for (const source of doasTempSources) {
        if (metricsInput[source] !== undefined && !isNaN(parseFloat(metricsInput[source]))) {
            const temp = parseFloat(metricsInput[source]);
            console.log(`[Element Logic Factory] Using temperature from metrics.${source}: ${temp}°F for ${equipmentType}`);
            return temp;
        }
    }

    console.log(`[Element Logic Factory] No temperature found for ${equipmentType}, using default: 65°F`);
    return 65;
}

// Helper function to gather state storage
async function gatherStateStorage() {
    try {
        return {
            lastControlUpdate: Date.now(),
            pidIntegral: 0,
            pidDerivative: 0,
            lastError: 0,
            pidState: null,
            gasValvePosition: 0,
            coolingStages: { stage1: false, stage2: false }
        };
    } catch (error) {
        console.error(`[Element Logic Factory] Error gathering state storage:`, error);
        return {};
    }
}

// Extract actionable commands from results - DOAS specific
function extractActionableCommands(result) {
    const actionable = {};

    // DOAS FAN COMMANDS
    if (result.fanEnabled !== undefined) actionable.fanEnabled = result.fanEnabled;

    // DOAS HEATING COMMANDS
    if (result.heatingEnabled !== undefined) actionable.heatingEnabled = result.heatingEnabled;
    if (result.gasValvePosition !== undefined) actionable.gasValvePosition = result.gasValvePosition;
    if (result.gasValveVoltage !== undefined) actionable.gasValveVoltage = result.gasValveVoltage;

    // DOAS COOLING COMMANDS
    if (result.coolingEnabled !== undefined) actionable.coolingEnabled = result.coolingEnabled;
    if (result.dxStage1Enabled !== undefined) actionable.dxStage1Enabled = result.dxStage1Enabled;
    if (result.dxStage2Enabled !== undefined) actionable.dxStage2Enabled = result.dxStage2Enabled;

    // TEMPERATURE AND STATUS
    if (result.supplyTemp !== undefined) actionable.supplyTemp = result.supplyTemp;
    if (result.outdoorTemp !== undefined) actionable.outdoorTemp = result.outdoorTemp;
    if (result.targetSetpoint !== undefined) actionable.targetSetpoint = result.targetSetpoint;
    if (result.controlMode !== undefined) actionable.controlMode = result.controlMode;
    if (result.temperatureError !== undefined) actionable.temperatureError = result.temperatureError;

    // SAFETY AND LOCKOUT STATUS
    if (result.heatingLockout !== undefined) actionable.heatingLockout = result.heatingLockout;
    if (result.coolingLockout !== undefined) actionable.coolingLockout = result.coolingLockout;
    if (result.emergencyShutdown !== undefined) actionable.emergencyShutdown = result.emergencyShutdown;

    return actionable;
}

// Write equipment results to NeuralControlCommands - HUNTINGTON PATTERN
async function writeEquipmentResults(equipmentType, results, equipmentId) {
    try {
        const database = 'NeuralControlCommands';
        const influxUrl = process.env.INFLUXDB_URL || 'http://143.198.162.31:8181';

        console.log(`[Element Logic Factory] Writing results to ${database} for ${equipmentType} (${equipmentId})`);
        console.log(`[DEBUG] writeEquipmentResults called with ${results.length} results`);

        for (const result of results) {
            console.log(`[DEBUG] Processing result:`, result);

            if (!result || typeof result !== 'object') {
                console.log(`[Element Logic Factory] Skipping invalid result:`, result);
                continue;
            }

            const actionableCommands = extractActionableCommands(result);
            if (Object.keys(actionableCommands).length === 0) {
                console.log(`[Element Logic Factory] No actionable commands found in result:`, result);
                continue;
            }

            console.log(`[Element Logic Factory] Extracted ${Object.keys(actionableCommands).length} commands:`,
                Object.keys(actionableCommands).join(', '));

            // Create individual line protocol entries for each command
            const lineProtocolEntries = [];

            for (const [commandType, value] of Object.entries(actionableCommands)) {
                // Convert all values to strings for the Boolean column
                const formattedValue = `"${String(value)}"`;

                // Create line protocol entry - MATCH HUNTINGTON FORMAT
                const lineProtocol = `NeuralCommands,equipment_id=${equipmentId},location_id=8,command_type=${commandType},equipment_type=${equipmentType},source=element-logic-factory,status=active value=${formattedValue}`;
                console.log(`[DEBUG] Line protocol: ${lineProtocol}`);
                lineProtocolEntries.push(lineProtocol);
            }

            // Send all commands in one batch
            if (lineProtocolEntries.length > 0) {
                console.log(`[DEBUG] About to send ${lineProtocolEntries.length} entries to InfluxDB`);
                const batchData = lineProtocolEntries.join('\n');

                try {
                    const response = await fetch(`${influxUrl}/api/v3/write_lp?db=${database}&precision=nanosecond`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'text/plain' },
                        body: batchData
                    });

                    if (response.ok) {
                        console.log(`[Element Logic Factory] Successfully wrote ${lineProtocolEntries.length} commands to ${database}`);
                    } else {
                        const errorText = await response.text();
                        console.error(`[Element Logic Factory] Write failed: ${response.status} - ${errorText}`);
                    }
                } catch (fetchError) {
                    console.error(`[Element Logic Factory] Fetch error writing to ${database}:`, fetchError);
                }
            }
        }
    } catch (error) {
        console.error(`[Element Logic Factory] Write error:`, error);
    }
}

// Worker event handlers
elementLogicWorker.on('completed', (job) => {
    console.log(`[Element Logic Factory] Job ${job.id} completed successfully`);
});

elementLogicWorker.on('failed', (job, err) => {
    console.error(`[Element Logic Factory] Job ${job.id} failed:`, err);
});

elementLogicWorker.on('error', (err) => {
    console.error('[Element Logic Factory] Worker error:', err);
});

// Graceful shutdown
async function shutdown() {
    console.log('[Element Logic Factory] Shutting down...');
    await elementLogicWorker.close();
    await connection.quit();
    process.exit(0);
}

process.on('SIGTERM', shutdown);
process.on('SIGINT', shutdown);

console.log('[Element Logic Factory] Element Logic Factory Worker started - Processing DOAS equipment');
console.log('[Element Logic Factory] Equipment supported: DOAS-1 (WBAuutoHnGUtAEc4w6SC), DOAS-2 (CiFEDD4fOAxAi2AydOXN)');
console.log('[Element Logic Factory] Queue: equipment-logic-8, Concurrency: 3, Location ID: 8');
console.log('[Element Logic Factory] Redis connected');
