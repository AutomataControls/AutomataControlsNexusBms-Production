...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (recipients && Array.isArray(recipients) && recipients.length > 0) {
      ...
      console.log(`📧 Using ${emailRecipients.length} provided recipients`)
    } else {
      ...
      console.log(`📧 No recipients provided, using default recipient: ${defaultRecipient}`)
      emailRecipients.push(defaultRecipient)
    }

    // Remove duplicates
    emailRecipients = [...new Set(emailRecipients)]
    console.log(`📧 Final recipient count after removing duplicates: ${emailRecipients.length}`)
    ...
    console.log(`📧 Sending email to ${emailRecipients.length} recipients: ${emailRecipients.join(", ")}`)
