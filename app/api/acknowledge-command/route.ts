...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Acknowledging command:", { commandId, commandType, sequentialId, equipmentId, locationId })
    ...
    // In InfluxDB, we can't directly update points
    // Instead, we'll write a new point with the same tags but with an "acknowledged" status
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json({
          success: true,
          warning: "Command may not have been acknowledged in InfluxDB. Please check server logs.",
        })
      }

      console.log("Successfully wrote acknowledgment to InfluxDB")
      return NextResponse.json({ success: true })
    } catch (writeError) {
      console.error("Error writing acknowledgment to InfluxDB:", writeError)
      // Return success anyway to allow UI to function
      return NextResponse.json({
        success: true,
        warning: "Command may not have been acknowledged in InfluxDB. Please check server logs.",
    ...
    console.error("Error acknowledging command in InfluxDB:", error)
    ...
    return NextResponse.json({
      success: true,
      warning: "An error occurred while acknowledging the command. Please check server logs.",
    })
  }
}
