...
export function boilerControl(metrics: any, settings: any, currentTemp: number, stateStorage: any) {
  ...
  const supplyTemp =
    metrics.H20Supply || // Primary field name from InfluxDB - EXACT MATCH (note: H20 not H2O)
  ...
  const returnTemp =
    metrics.H20Return || // Primary field name from InfluxDB - EXACT MATCH (note: H20 not H2O)
  ...
  const outdoorTemp =
    metrics.outdoorTemp || // Primary field name from InfluxDB - EXACT MATCH
