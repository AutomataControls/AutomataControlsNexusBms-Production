 ...
 */
export function airHandlerControl(metrics: any, settings: any, currentTemp: number, pidState: any): any {
  ...
  logLocationEquipment(locationId, equipmentId, "air-handler", "Starting Warren-specific air handler control logic");
  ...
  if (settings.controlSource !== settings.temperatureSource) {
    logLocationEquipment(locationId, equipmentId, "air-handler",
      `WARNING: Control source (${settings.controlSource}) doesn't match temperature source (${settings.temperatureSource}). Overriding with ${controlSource} for AHU-${ahuNumber}.`);
  }
  ...
  if (ahuNumber === 2) {
    ...
    heatingStage1Status = metrics["Heating_Stage_1_Status"] || metrics["Heating Stage 1 Status"] || false;
    heatingStage1Amps = metrics["Heating_Stage_1_Amps"] || metrics["Heating Stage 1 Amps"] || 0;
    heatingStage2Status = metrics["Heating_Stage_2_Status"] || metrics["Heating Stage 2 Status"] || false;
    heatingStage2Amps = metrics["Heating_Stage_2_Amps"] || metrics["Heating Stage 2 Amps"] || 0;
  ...
  if (currentTemp === undefined) {
    if (controlSource === "supply") {
      ...
      logLocationEquipment(locationId, equipmentId, "air-handler",
        `Using supply air temperature: ${currentTemp}°F from source "${tempSource}" (for AHU-${ahuNumber})`);
    } else {
      // For space-controlled AHUs: Use space temperature with all Warren-specific room sensors
      let tempSource = "default";

      // DEBUG: Log all available metrics for AHU-2
      if (ahuNumber === 2) {
        logLocationEquipment(locationId, equipmentId, "air-handler",
          `AHU-2 DEBUG: All available metrics keys: ${Object.keys(metrics).join(', ')}`);
        
        // Check specific space temperature fields
        const spaceFields = ['Space', 'spaceTemperature', 'SpaceTemp', 'spaceTemp', 'SpaceTemperature'];
        spaceFields.forEach(field => {
          if (metrics[field] !== undefined) {
            logLocationEquipment(locationId, equipmentId, "air-handler",
              `AHU-2 DEBUG: Found ${field} = ${metrics[field]}`);
          }
        });
      }
      ...
      // DEBUG: Log what we actually got for currentTemp for AHU-2
      if (ahuNumber === 2) {
        logLocationEquipment(locationId, equipmentId, "air-handler",
          `AHU-2 DEBUG: Final currentTemp = ${currentTemp}, tempSource = ${tempSource}`);
      }

      logLocationEquipment(locationId, equipmentId, "air-handler",
        `Using space temperature: ${currentTemp}°F from source "${tempSource}" (for AHU-${ahuNumber})`);
    }
  }
  ...
  if (userSetpoint !== undefined) {
    ...
    if (outdoorTemp <= minOAT) {
      ...
      logLocationEquipment(locationId, equipmentId, "air-handler",
        `OAR: OAT ${outdoorTemp}°F <= ${minOAT}°F, using max ${controlSource} setpoint: ${setpoint}°F`);
    } else if (outdoorTemp >= maxOAT) {
      setpoint = minSetpoint;
      logLocationEquipment(locationId, equipmentId, "air-handler",
        `OAR: OAT ${outdoorTemp}°F >= ${maxOAT}°F, using min ${controlSource} setpoint: ${setpoint}°F`);
    } else {
      ...
      logLocationEquipment(locationId, equipmentId, "air-handler",
        `OAR: Calculated ${controlSource} setpoint: ${setpoint}°F (ratio: ${ratio.toFixed(2)})`);
    }
  }
  ...
  if (controlSource === "space") {
    logLocationEquipment(locationId, equipmentId, "air-handler",
      `Current space temperature: ${currentTemp}°F (Current setpoint: ${setpoint}°F, Error: ${(setpoint - currentTemp).toFixed(1)}°F)`);
  } else {
    logLocationEquipment(locationId, equipmentId, "air-handler",
      `Current supply temperature: ${currentTemp}°F (Current setpoint: ${setpoint}°F, Error: ${(setpoint - currentTemp).toFixed(1)}°F)`);
  }
  ...
  if (safetyTripped) {
    ...
    if (safetyReason === "freezestat") {
      ...
      logLocationEquipment(locationId, equipmentId, "air-handler",
        "Freezestat response: HW valve open, fan disabled, OA dampers closed");
    }
    ...
    if (safetyReason === "highlimit") {
      ...
      logLocationEquipment(locationId, equipmentId, "air-handler",
        "High limit response: HW valve closed, CW valve open");
    }
  ...
  if (outdoorTemp > 40 && !safetyTripped && isOccupied) {
    ...
    if (outdoorTemp <= 40) {
      ...
      logLocationEquipment(locationId, equipmentId, "air-handler",
        `OA damper: CLOSED (unoccupied mode)`);
    } else {
      logLocationEquipment(locationId, equipmentId, "air-handler",
        `OA damper: CLOSED (safety tripped: ${safetyReason})`);
    }
  }
  ...
  if (ahuNumber === 4) {
    ...
    if (ahuNumber === 7) {
      ...
      const preheatOutput = pidControllerImproved({
        input: currentTemp,
        setpoint: setpoint,
        pidParams: {
          kp: 1.2,
      ...
      const reheatOutput = pidControllerImproved({
        input: currentTemp,
        setpoint: setpoint,
        pidParams: {
          kp: 0.8,
      ...
      logLocationEquipment(locationId, equipmentId, "air-handler",
        `AHU-7: Preheat output: ${preheatOutput.output.toFixed(1)}%, Reheat output: ${reheatOutput.output.toFixed(1)}%`);
      ...
      logLocationEquipment(locationId, equipmentId, "air-handler",
        `Final control values for AHU-7: fan=${ahu7Result.fanEnabled ? "ON" : "OFF"} (${fanAmps.toFixed(2)}A), ` +
        `heating=${ahu7Result.heatingValvePosition.toFixed(1)}%, cooling=${ahu7Result.coolingValvePosition}%, ` +
        `damper=${ahu7Result.outdoorDamperPosition}%`,
        ahu7Result);
  ...
  if (ahuNumber === 2) {
    ...
    if (shouldEnableElectricHeat) {
      ...
      if (heatingStage1Command || heatingStage2Command) {
        ...
        logLocationEquipment(locationId, equipmentId, "air-handler",
          `Reducing hydronic heating due to active electric heat: ${originalHeatingValve.toFixed(1)}% → ${heatingValvePosition.toFixed(1)}%`);
      }
    } else {
      logLocationEquipment(locationId, equipmentId, "air-handler",
    ...
    logLocationEquipment(locationId, equipmentId, "air-handler",
      `Final control values for AHU-2: fan=${ahu2Result.fanEnabled ? "ON" : "OFF"} (${fanAmps.toFixed(2)}A), ` +
  ...
  if (controlSource === "supply" && currentTemp !== supplyTemp) {
    logLocationEquipment(locationId, equipmentId, "air-handler",
      `WARNING: AHU-${ahuNumber} should use supply temp but is using ${currentTemp}°F. Correcting to supply temp ${supplyTemp}°F`);
  ...
  if (!fanEnabled) {
    ...
    logLocationEquipment(locationId, equipmentId, "air-handler",
      `Final control values (unoccupied): fan=OFF, heating=0%, cooling=0%, damper=0%`,
      unoccupiedResult);
  ...
  const baseResult = airHandlerControlBase(metrics, warrenSettings, currentTemp, pidState);
