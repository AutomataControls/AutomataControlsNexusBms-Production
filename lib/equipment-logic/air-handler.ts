  ...
  if (operationMode === "cooling") {
    ...
    if (pidSettings.cooling.enabled) {
      ...
      console.log(
        ...
        ).toFixed(2)}, P=${coolingPID.p.toFixed(2)}, I=${coolingPID.i.toFixed(2)} (max=${
          pidSettings.cooling.maxIntegral
        }), D=${coolingPID.d.toFixed(2)}, Output=${coolingPID.output.toFixed(2)}`
      )
      ...
      console.log(
        `Cooling: Temp diff (current - setpoint) = ${tempDiff.toFixed(2)}°F, Valve position = ${coolingValvePosition.toFixed(2)}%`
      )
      
      if (tempDiff > 0) {
        console.log(
          `Supply temp too warm (${currentTemp}°F > ${supplySetpoint}°F): Opening cooling valve to ${coolingValvePosition.toFixed(2)}%`
        )
      } else if (tempDiff < 0) {
        console.log(
          `Supply temp too cool (${currentTemp}°F < ${supplySetpoint}°F): Closing cooling valve to ${coolingValvePosition.toFixed(2)}%`
        )
      }
    ...
    if (pidSettings.heating.enabled) {
      ...
      console.log(
        ...
        ).toFixed(2)}, P=${heatingPID.p.toFixed(2)}, I=${heatingPID.i.toFixed(2)} (max=${
          pidSettings.heating.maxIntegral
        }), D=${heatingPID.d.toFixed(2)}, Output=${heatingPID.output.toFixed(2)}`
      )
      ...
      console.log(
        `Heating: Temp diff (setpoint - current) = ${tempDiff.toFixed(2)}°F, Valve position = ${heatingValvePosition.toFixed(2)}%`
      )
      
      if (tempDiff > 0) {
        console.log(
          `Supply temp too cool (${currentTemp}°F < ${supplySetpoint}°F): Opening heating valve to ${heatingValvePosition.toFixed(2)}%`
        )
      } else if (tempDiff < 0) {
        console.log(
          `Supply temp too warm (${currentTemp}°F > ${supplySetpoint}°F): Closing heating valve to ${heatingValvePosition.toFixed(2)}%`
        )
      }
